...
describe DiscourseImageEnhancement::ImageSearch do
  ...
  describe "can search image" do
    ...
    let!(:image_search_data1) do
      ImageSearchData.create(sha1: image_upload1.sha1, upload_id: image_upload1.id, ocr_text: "car", ocr_text_search_data: "'car':2", embeddings: Array.new(512) { rand }.to_s)
    end
    let!(:image_search_data2) do
      ImageSearchData.create(sha1: image_upload2.sha1, upload_id: image_upload2.id, ocr_text: "plane", ocr_text_search_data: "'plane':2", embeddings: Array.new(512) { rand }.to_s)
    end
    let!(:post) { Fabricate(:post, topic: Fabricate(:topic), uploads: [image_upload1, image_upload2]) }
    ...
    it "searches using embeddings only" do
      ...
      allow(DiscourseImageEnhancement::TextEmbedding).to receive(:embed_text).and_return(JSON.parse(image_search_data1.embeddings))
      result = search.execute
      expect(result.grouped_results.map(&:image).map(&:id)).to include(image_upload1.id)
