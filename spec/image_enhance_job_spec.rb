...
describe ::Jobs::DiscourseImageEnhancement::AutoCleanup do
  ...
  it 'should cleanup orphaned failed_count' do
    PluginStore.set('discourse-image-enhancement', "failed_count", {image_upload.sha1 => 4, "5678" => 100})
    image_upload
    image_search_data.destroy
    described_class.new.execute({})
    expect(PluginStore.get('discourse-image-enhancement', "failed_count")).to eq({image_upload.sha1 => 4})
  end
